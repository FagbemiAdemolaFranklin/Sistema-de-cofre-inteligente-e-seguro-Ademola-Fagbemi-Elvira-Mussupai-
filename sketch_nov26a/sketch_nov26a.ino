...
void setPassword(){
  ...
  if(inserted_new_password_1.length() < 4) {
    ...
    is_typing = true;
    Serial.write("is_typing"); // this is to make the connection open for setting password so as to print whatever is being typed on the lcd.
    String new_key_string(new_key);
    Serial.write(new_key);
    inserted_new_password_1 += new_key_string;
  }else if(inserted_new_password_1.length() == 4){
    // lcd.home();
    // lcd.write("Confirm password");
    Serial.write("is confirming password"); //  this is a signal to the lcd on arduino 2, expected to show confirming password on lcd.
    if(new_key){
      // k++;
      // lcd.setCursor(k,1);
      // lcd.write(new_key);
      ...
      Serial.write(new_key); // each chharacter of the confirm password is sent to the second arduino so as to be printed on the screen.
...
void openDoor(String inputed_key) {
  ...
  Serial.write("Writing"); // print each chracters inserted
  if(inserted_password.length() == 4) {
    if(inserted_password.equals(my_password)){
      servo.write(0);
      // lcd.clear();
      // lcd.write("Opened");
      Serial.write("Open Door"); // open door displayed on the screen.
...
void loop()                     // run over and over again
{
  ...
  while (code_try <= 2) {
    ...
    Serial.write("Enter password"); // lcd must print enter password
